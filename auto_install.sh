#!/usr/bin/env bash
# Auto installation wrapper for termux_security_toolkit.sh
# Adds defensive bash options and consistent formatting.
...
install_core_packages() {
    ...
    if ! command -v pkg >/dev/null 2>&1; then PKG_MANAGER="apt"; fi
    $PKG_MANAGER update -y || true
    $PKG_MANAGER upgrade -y || true
    $PKG_MANAGER install -y git wget curl python ruby tsu sudo nmap hydra aircrack-ng metasploit hashcat john sqlmap nikto || true
...
echo -e "${CYAN}[✓] No-Auth Mode enabled - authentication prompts bypassed${RESET}"
